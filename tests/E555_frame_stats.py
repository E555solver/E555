...
N_RB = 3          # row bands: near = rows 0-2, mid = 3-12, far = 13-15
...
def main(argv=None):
    ...
    # -- how much far-side material a board leaves behind -----------------------
    # The exclusion A/B asked "does barring these pieces help the beam". The other
    # question, which the A/B cannot answer, is whether boards that happen to
    # LEAVE them alone are better. That needs a per-board count, which the
    # per-config tensors above have already averaged away, so it takes one extra
    # streaming pass -- cheap, and only when there is a list to count.
    reserve = None
    if exclude:
        ex = np.array(exclude)
        per_board, per_border = [], defaultdict(list)
        for _i, cid, _sol, pos, _rot in V.iter_records(args.corpus):
            if side_of(cid) != 0:            # the canonical growth direction only
                continue
            k = int((np.asarray(pos)[ex] == UNPLACED).sum())
            per_board.append(k)
            per_border[border_of(cid)].append(k)
        if per_board:
            pb = np.array(per_board, float)
            bm = np.array([np.mean(v) for v in per_border.values()])
            within = float(np.mean([np.var(v) for v in per_border.values() if len(v) > 1]))
            # What an indifferent placement would leave: each far piece unplaced
            # with the same probability as any inner piece, i.e. the share of
            # inner pieces that rows 1..stop_row do not consume.
            n_unplaced = 196 - (args.ab_stop_row - 2) * 14
            expect = len(exclude) * n_unplaced / 194.0
            reserve = {
                "n_boards": len(per_board), "n_borders": len(per_border),
                "list_size": len(exclude),
                "mean": round(float(pb.mean()), 3),
                "expected_if_indifferent": round(expect, 3),
                "hist": {str(k): int((pb == k).sum()) for k in range(len(exclude) + 1)},
                "frac_ge_8": round(float((pb >= 8).mean()), 5),
                "between_frac": round(float(bm.var() / max(pb.var(), 1e-12)), 4),
                "within_var": round(within, 3),
            }
            print(f"[reserve] side-0 boards leave {reserve['mean']:.2f} of "
                  f"{len(exclude)} far-side pieces unplaced "
                  f"(indifferent would be {expect:.2f}); "
                  f"{reserve['frac_ge_8']:.2%} leave 8 or more")
            print(f"[reserve] {reserve['between_frac']:.0%} of the variance is "
                  f"BETWEEN borders -- the rest is decided board by board")
