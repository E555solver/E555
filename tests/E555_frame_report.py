...
<section>
  ...
  <div class="col">
    ...
    # Measured by the stats pass: how much far-side material an ordinary board
    # already leaves behind. It decides whether the elite property has to be
    # forced by exclusion or can simply be selected for.
    RS = S.get("reserve")
    if RS:
        hist = RS["hist"]
        bars = "".join(
            f"<tr><td class='num'>{k}</td><td class='num'>{v:,}</td>"
            f"<td class='num'>{v/RS['n_boards']:.2%}</td></tr>"
            for k, v in sorted(hist.items(), key=lambda kv: int(kv[0])) if v)
        reserve_block = f"""
    <p>But the corpus says the property may not need forcing at all. Over
    {RS['n_boards']:,} side-0 boards, counting how many of the
    {RS['list_size']} far-side pieces each one left unplaced:</p>
    <div class="tbl-scroll"><table>
      <tr><th>far-side pieces left unplaced</th><th>boards</th><th>share</th></tr>
      {bars}
    </table></div>
    <div class="callout">
      <span class="eyebrow">two numbers that change the design</span>
      <p>The beam already leaves <strong>{RS['mean']:.2f}</strong> of
      {RS['list_size']} unplaced against <strong>
      {RS['expected_if_indifferent']:.2f}</strong> expected under indifferent
      placement, and <strong>{RS['frac_ge_8']:.1%}</strong> of boards leave eight
      or more. Elite boards therefore already exist in ordinary output, and
      selecting them costs <em>zero search time</em> &mdash; where forcing the
      property cost roughly 100&times; the throughput.</p>
      <p>And <strong>{1-RS['between_frac']:.0%} of the variance is decided board
      by board</strong>, not by the border ({RS['between_frac']:.0%} between
      borders). You cannot pick good borders for this; you have to pick good
      boards. Which also means <code>--max_per_config</code> is working against
      you here &mdash; it caps exactly the distribution you would want to sample
      deeply.</p>
    </div>"""
    else:
        reserve_block = ""
...
<section>
  <span class="sec-num">08 &middot; the question this did not answer</span>
  <div class="col">
    <h2>Few boards may be the point, not the problem</h2>
    <p>The A/B above measured <strong>P(board emitted)</strong>. It did not
    measure <strong>P(completable | emitted)</strong>, and those are different
    questions. Stage C is the bottleneck, not Stage B &mdash; there are already
    thousands of high-scoring partials and no way to focus on the few worth
    CP-SAT time. If exclusion yields few boards that are far more completable,
    that is a win nothing above could have seen.</p>
    <p>The mechanism is a real blind spot. The beam's objective scores
    <em>colour supply</em>, not <em>which specific pieces remain</em>. A board can
    finish row 11 with a perfect colour ledger and still be unfinishable, because
    it spent the only pieces that could ever sit at some cell in row 14. Exclusion
    closes that by brute force.</p>
    {reserve_block}
  </div>
</section>

<section>
  <span class="sec-num">09 &middot; before any of this reaches Stage C</span>
  <div class="col">
    <h2>The prior does not transfer as measured</h2>
    <p>The study's frame is orientation 0 with corners
    <span class="mono">BL=3 BR=2 TL=0 TR=1</span>. Checked against the real boards
    in <code>data/</code>: <strong>not one matches it</strong>, and most carry no
    clue commitment at all &mdash; they came from free runs. Three of the four
    boards in <code>best_463.csv</code> satisfy zero clues, and every one uses a
    different corner assignment.</p>
    <p>So a prior phrased as &ldquo;piece 124 prefers the top-right of the
    published board&rdquo; is unusable on that corpus. The fix costs nothing:
    the preference is not really about board geometry, it is about
    <em>which pinned pieces a corner carries</em>. In this frame TR holds
    corner-piece 1, BL holds 3, BR holds 2, TL holds 0 &mdash; so re-key the whole
    table to</p>
    <pre>piece 124 wants the corner occupied by <b>corner-piece 1</b></pre>
    <p>which is frame-independent, because the four corner pieces are
    identifiable in any board. Caveat worth carrying: each corner here is anchored
    by <em>both</em> a corner piece and a nearby clue, so re-keying keeps the
    corner anchor and drops the clue anchor. Expect the transferred prior to be
    weaker than the measured one &mdash; and that is measurable, by re-running the
    replication check keyed that way.</p>
  </div>
</section>

<section>
  <span class="sec-num">10 &middot; an aside worth keeping</span>
  <div class="col">
...
<section>
  <span class="sec-num">11 &middot; limits</span>
  <div class="col">
