...
frame, and the old top frame row lands inside the board with its grey pointing
the wrong way. Both come free, which is also how all four corners do.

The run prints the breaks left among the pieces that survived, and that is the
number to choose N by:
...
Six of the seven shipped 463s keep a perfectly matched 176-piece core at
`--sink 3`, and only two of them do at `--sink 2`. A sunk board has holes in its
frame, so it is a partial: it goes to Stage C, not back to the beamer.
