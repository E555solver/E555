...
# missing and the backtracker fills that in. A longer --max_wall_sec in stage 1
...
"$REPO/bin/E555_beamer" "$REPO/data/seed_Edge5.txt" \
    ...
    --max_partials 25 --border_row_N 50 --top_columns 1 --max_wall_sec 600 \
    --threads "$THREADS" --out_dir "${PREFIX}_beam" --print-cmd
...
# --border_row_N 0 means every line of the input CSV, whatever the last stage
...
"$REPO/bin/E555_finalizer" "$REPO/data/seed_Edge5.txt" "${PREFIX}_beam12.csv" \
    ...
    --finalize_repeats 2 --border_row_N 0 \
    --threads "$THREADS" --out_dir "${PREFIX}_fin" --print-cmd
...
if [ -s "${PREFIX}_full12.csv" ]; then
    ...
    "$REPO/bin/E555_roundhouse" "$REPO/data/seed_Edge5.txt" "${PREFIX}_full12.csv" \
        --rotate -1 --rounds 3 --strip_width 5 --border_row_N 0 \
        --max_wall_sec 600 --threads "$THREADS" \
        --out_dir "${PREFIX}_rh" --print-cmd
else
    echo "    nothing reached a complete row 12, so there is nothing to spiral."
    echo "    Raise --max_wall_sec on stage 1 and this stage starts doing work."
fi
...
"$REPO/bin/E555_backtracker" "$REPO/data/seed_Edge5.txt" \
    ...
    --max-mismatch 20 --time-limit 20 --threads "$THREADS" --print-cmd
...
python3 "$REPO/tools/E555_rank.py" "${PREFIX}_solved.csv" \
    --seed "$REPO/data/seed_Edge5.txt" --rescore --emit "${PREFIX}_ranked.csv"
...
python3 "$REPO/tools/E555_viewer.py" "${PREFIX}_ranked.csv" \
    --seed "$REPO/data/seed_Edge5.txt" --row 0
