...
and writes the Stage C commands to attack them; add `--triage` to rank a six-figure
corpus in one streaming pass first. And when a stage keeps stalling on the same
...
| triage | **`E555_distiller.py`** (Python) | Distils thousands of >=450 boards down to the few worth Stage C time. Ranks by what a board could *become*, not what it is: the cheapest repair window covering its own breaks, the single-swap escape routes left inside it, the colour ledger the beamer scores rows with, and how good randomized dives get there. `--plan` writes the hole masks and a runnable `run_plan.sh`; `--triage` is a streaming first cut for a corpus of any size. No tuning knobs. |
| triage | **`E555_extract_consensus.py`** (Python) | Ranks a large pool of CLUED partials by agreement with the pool itself. The five clues are one rigid body in four configurations, so a clued board's orientation can be read and every board turned into one frame; the piece-by-cell frequency table that follows is the *consensus*, and a board is scored in bits above chance against the whole frequency column, not just its mode. `--best_top`/`--best_bottom` ask the other question -- never mind where this board's pieces are, is the bag it has LEFT the right bag for the rows it has left -- and `--border_out` distils the same table into a Stage A rotations row the beamer reads, repaired with the annealer's Euler machinery so every side stays rich in trails. |
| power tool | **`E555_ender.py`** (CP-SAT) | The closer. Opens a slice of a full board and re-solves it under a move budget to drive breaks toward zero, never returning a worse board. Pick an effort `--profile` and a true `--board_time_limit`; it chooses its own neighbourhoods, small break-centred pools first and broad ones after. |
